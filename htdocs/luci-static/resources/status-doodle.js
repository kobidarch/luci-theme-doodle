...
return baseclass.extend({
  ...
  __init__: function () {
    ...
    try {
      if (
        window.doodle_speed_unit === "bytes" ||
        window.doodle_speed_unit === "bits"
      )
  ...
  icons: {
    ...
    uptime:
      '<svg xmlns="http://www.w3.org/2000/svg" class="indicator-icon" width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="7" cy="7" r="6"/><polyline points="7 3 7 7 10 9"/></svg>',
  },
  ...
  createIndicator: function (name, title) {
    ...
    var cached = null;
    try {
      cached = sessionStorage.getItem("doodle-status-" + name);
    } catch (e) {}
  ...
  resolvePhysicalDevice: function (devName) {
    ...
    L.resolveDefault(callDeviceStatus(devName), {}).then(function (dev) {
      ...
      L.resolveDefault(callDeviceStatus(self.netDevice), {}).then(
        function (phys) {
          if (phys && phys.speed) {
  ...
  updateSystem: function (info) {
    ...
    if (info.load) {
      ...
      try {
        sessionStorage.setItem("doodle-status-cpu", pctStr);
      } catch (e) {}
      this.cpuEl.title =
        "CPU: " +
        pctStr +
    ...
    if (info.memory) {
      ...
      try {
        sessionStorage.setItem("doodle-status-ram", pct + "%");
      } catch (e) {}
      this.ramEl.title =
        "RAM: " +
    ...
    if (info.uptime) {
      this.uptimeEl.querySelector(".indicator-value").textContent =
        this.formatUptime(info.uptime);
      try {
        sessionStorage.setItem(
          "doodle-status-uptime",
          this.formatUptime(info.uptime),
        );
      } catch (e) {}
  ...
  updateNetwork: function (dev) {
    ...
    if (this.prevStats && this.prevTime) {
      ...
      if (dt > 0) {
        ...
        var netText =
          "\u2193" +
          this.formatSpeed(rxSpeed) +
          " \u2191" +
          this.formatSpeed(txSpeed);
        this.netEl.querySelector(".indicator-value").textContent = netText;
        try {
          sessionStorage.setItem("doodle-status-net", netText);
        } catch (e) {}
        var tip =
          (this.netLabel || this.netDevice) +
          ": \u2193 " +
          this.formatSpeedFull(rxSpeed) +
          " / \u2191 " +
          this.formatSpeedFull(txSpeed);
        if (this.linkSpeed)
          tip +=
            " (Link: " +
            (this.linkSpeed >= 1000
              ? this.linkSpeed / 1000 + " Gbps"
              : this.linkSpeed + " Mbps") +
            ")";
