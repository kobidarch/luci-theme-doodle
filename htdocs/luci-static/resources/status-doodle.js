...
return baseclass.extend({
	...
	createIndicator: function(name, title) {
		...
		var cached = null;
		try { cached = sessionStorage.getItem('doodle-status-' + name); } catch(e) {}
	...
	updateSystem: function(info) {
		...
		if (info.load) {
			...
			try { sessionStorage.setItem('doodle-status-cpu', pctStr); } catch(e) {}
		...
		if (info.memory) {
			...
			try { sessionStorage.setItem('doodle-status-ram', pct + '%'); } catch(e) {}
		...
		if (info.uptime) {
			...
			try { sessionStorage.setItem('doodle-status-uptime', this.formatUptime(info.uptime)); } catch(e) {}
	...
	updateNetwork: function(dev) {
		...
		if (this.prevStats && this.prevTime) {
			...
			if (dt > 0) {
				...
				try { sessionStorage.setItem('doodle-status-net', netText); } catch(e) {}
